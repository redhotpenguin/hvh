...
sub setup {
    ...
    $self->run_modes( [qw( form bookit contact hold )] );
}
...
sub check_booking {
    ...
    my $sql =
"Select Id from Booking__c where (Booking_Stage__c != 'Dead' and Booking_Stage__c != 'Working' and Booking_Stage__c != 'Unqualified' )  and Property_name__c = '$prop_id' and ( ";
...
sub form {
    ...
    return 'not found' unless ($q->param('prop_id') && $mode);

    my %tmpl = ( prop_id => $q->param('prop_id'));
    if ( keys %{$errors} ) {
	%tmpl = ( %tmpl, %{$errors});
    }
    my $output = $self->tt_process("$mode\.tmpl", \%tmpl);
