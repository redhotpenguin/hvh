...
sub render {
  ...
  my $text = <<SCRIPT;
    ...
    if (GBrowserIsCompatible()) {
      var $mapid = new GMap2(document.getElementById("$this->{id}"));
SCRIPT
  $text .= "      $mapid.setCenter(new GLatLng($this->{center}[0], $this->{center}[1]));\n"
    if $this->{center};
  $text .= "      $mapid.setZoom($this->{zoom});\n"
    if $this->{zoom};

  $text .= "      $mapid.setMapType($this->{type});\n";
    
  if ($this->{controls}) {
    foreach my $control (@{$this->{controls}}) {
      $control =~ s/_(.)/uc($1)/ge;
      $control = ucfirst($control);
      $text .= "      $mapid.addControl(new G${control}());\n";
    }
  }
  unless ($this->{dragging}) {
    $text .= "      $mapid.disableDragging();\n";
  }
  ...
  foreach my $point (@{$this->{points}}) {
    ...
    if ($point->{format} && $point->{html}) {
      $point_html = sprintf(
        '<div style="width:180px;height:40px;">%s</div>',
        $point->{html},
      );
    }

    $text .= "      var $mapid\_marker_$i = new GMarker(new GLatLng($point->{point}[0], $point->{point}[1]) $icon);\n";
    $text .= "      GEvent.addListener($mapid\_marker_$i, \"click\", function () {  $mapid\_marker_$i.openInfoWindowHtml('$point_html'); });\n"
      if $point->{html};
    $text .= "      $mapid.addOverlay($mapid\_marker_$i);\n";
  }
  ...
  foreach my $polyline (@{$this->{poly_lines}}) {
    ...
    $text .= "      $mapid.addOverlay(polyline_$i);\n";
  }
